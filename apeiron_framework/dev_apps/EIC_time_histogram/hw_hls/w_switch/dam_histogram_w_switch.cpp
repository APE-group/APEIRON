...
#define BINWIDTH_INT 254 //((int)(2.54*4*TIME_SCALE/N_BINS))  // scaled

#define BITWIDTH_BIN 32  //128/N_BINS

#define FINE_MULT 3
#define COARSE_MULT 254

#define MAX_WORD_BUFFER 1024


typedef ap_uint<32> RDO_word_t;
typedef ap_uint<128> APE_word_t;

/*** Read message from INPUT_CHANNELS (32bit), store them in buffers, fill histogram and stream them to next step ***/
void fifo_reading(RDO_word_t stored_buff[MAX_WORD_BUFFER], unsigned &word_count, hls::stream<ap_uint<BITWIDTH_BIN*N_BINS>> &fifo_time_histo, hls::stream<RDO_word_t> &dam_channel_PDU){
#pragma HLS inline off
//#pragma HLS pipeline
	//bool event_done = false;
	ap_uint<BITWIDTH_BIN*N_BINS> time_histo=0;

	read_word_loop: while(true){
	#pragma HLS pipeline
		RDO_word_t input_word = dam_channel_PDU.read();
		stored_buff[word_count%MAX_WORD_BUFFER] = input_word;
		word_count++;
...
/*** Receive GTU bit, and flush stored buffers***/
void GTU_message_receive(hls::stream<bool> GTU_link_in[1], unsigned word_count[N_INPUT_CHANNELS], RDO_word_t word_buffer[N_INPUT_CHANNELS][MAX_WORD_BUFFER]){
#pragma HLS inline off
//#pragma HLS pipeline
	bool signal;
	GTU_link_in[0].read_nb(signal);
	if(signal){
flush_main_loop: for (int ch = 0; ch < N_INPUT_CHANNELS; ch++) {
		#pragma HLS unroll
flush_word_counted: for(int i=0; i<word_count[ch]; i++){
...
/*** Merge Histogram from each channel, then stream an APE_word to sector_histogram ***/
void APE_sending(hls::stream<ap_uint<BITWIDTH_BIN*N_BINS>> fifo_time_histo[N_INPUT_CHANNELS], hls::stream<APE_word_t> &message_data_out){
#pragma HLS inline off
//#pragma HLS pipeline
...
void dam_histogram(hls::stream<RDO_word_t> dam_channels_PDUs[N_INPUT_CHANNELS],
		hls::stream<bool> GTU_link_in[1],
		hls::stream<APE_word_t> message_data_out[N_OUTPUT_CHANNELS])
 {
	unsigned word_count[N_INPUT_CHANNELS] = {0};
	#pragma HLS ARRAY_PARTITION variable=word_count type=complete	

	RDO_word_t word_buffer[N_INPUT_CHANNELS][MAX_WORD_BUFFER] = {0};
	#pragma HLS ARRAY_PARTITION variable=word_buffer type=complete	
...
#pragma HLS dataflow
	...
	#pragma HLS unroll
    		 fifo_reading(word_buffer[ch],word_count[ch],fifo_time_histo[ch], dam_channels_PDUs[ch]);
	}
	//**** Histogram packing into APEword *****//
	APE_sending(fifo_time_histo, message_data_out[0]);

	//**** GTU signal for buffer flushing *****//
	GTU_message_receive(GTU_link_in, word_count, word_buffer);
}
}
