...
void APE_sending(unsigned ch_id, hls::stream<APE_word_t> fifo_histo_sum[N_INPUT_CHANNELS],  hls::stream<APE_word_t> message_data_out[N_OUTPUT_CHANNELS]){
#pragma HLS inline off
...
write_ch_loop: for(unsigned ch=0; ch<N_INPUT_CHANNELS; ch++){
		...
		word_t apeword = histo_sum[0] + histo_sum[1] + histo_sum[2] + histo_sum[3] + histo_sum[4];
		send(&apeword, sizeof(word_t), 0, 3, ch_id, message_data_out, false);
}
//}

extern "C" {

void sector_histogram_3( 
...
#pragma HLS dataflow
	...
	APE_sending(ch_id,fifo_histo_sum, message_data_out);
