...
int main(int argc, char** argv) {
	FILE *f;
    if (argc != 3) {
        std::cout << "Usage: " << argv[0] << " NB_IMAGE_REPETITIONS <XCLBIN File>" << std::endl;
        return EXIT_FAILURE;
    }
    ...
    in_vector_size_bytes = image_size_bytes;
    out_vector_size_bytes = image_size_bytes;
    
        unsigned int local_coord = 0;
    unsigned int dest_coord = 0;
    unsigned int mode = 1;
    ...
    char filename[255];
    char path[255] = "/apotto/home1/aliens/rossi/Downloads/";
    char extension[255] = "raw";
...
*/     xrt::device device(0);	
    ...
    sleep(1);
    kswitch.write_register(6*4,local_coord);
    
   kswitch.write_register(8*4,0xfffffc4);
    ...
    ImgRows = NB_ROWS[0];
    ImgCols = NB_COLS[0];
    unsigned int ImgSize = ImgRows*ImgCols;
...
*/	
	...
	krnl_Memory2RGB_1_run.set_arg(4, ImgSize);
	krnl_Memory2RGB_1_run.set_arg(5, NumberOfImageRepetitions);
	krnl_Memory2RGB_1_run.set_arg(6, mode);
	krnl_Memory2RGB_1_run.set_arg(7, dest_coord);
	krnl_Memory2RGB_1_run.set_arg(8, packet_size);

	krnl_RGB2Memory_1_run.set_arg(4, ImgSize);
	krnl_RGB2Memory_1_run.set_arg(5, NumberOfImageRepetitions);
	krnl_RGB2Memory_1_run.set_arg(6, packet_size);
...
/*
		...
		krnl_ImgProc_1_run.set_arg(10, 0);
		...
		krnl_ImgProc_2_run.set_arg(10, 1);
		krnl_ImgProc_2_run.set_arg(11, packet_size);
		
		krnl_ImgProc_3_run.set_arg(6, ImgSize);
		krnl_ImgProc_3_run.set_arg(7, NumberOfImageRepetitions);
		krnl_ImgProc_3_run.set_arg(8, ImgRows);
		krnl_ImgProc_3_run.set_arg(9, ImgCols);
		krnl_ImgProc_3_run.set_arg(10, 2);
		krnl_ImgProc_3_run.set_arg(11, packet_size);
*/	
...
//	save one of the processed images to file
    ...
    printf("computation ended. Writing output file\n");
    
    out_vector_size_bytes = image_size_bytes;
#ifdef _SAVE_OUTPUT_FILES_

	for (unsigned int i=0; i<1; i++)
	{
		sprintf(filename,"%sOut/%s_loopback.%s",path, name[i],extension);
		...
		if (i==0)
			fwrite(buffer_out_0_map,1,out_vector_size_bytes,f);
		else
			fwrite(buffer_out_1_map,1,out_vector_size_bytes,f);
		fclose(f);
	}
#endif
