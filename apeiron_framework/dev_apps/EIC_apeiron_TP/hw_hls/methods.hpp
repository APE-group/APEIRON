...
int receive(channel_id_t ch_id, ap_axis<128,0,0,0> *buff, 
		hls::stream<ap_axis<128,0,0,0>> &message_data_in){

	auto hdr = message_data_in.read(); 
	

	int size = hdr.data.range(79,66);

	for (short i = 0; i < size/sizeof(word_t); ++i){ 
	#pragma HLS pipeline
		  
		buff[i] = message_data_in.read(); 
	}

	auto ftr = message_data_in.read();  
...
size_t send(word_t *buff, size_t size, int coord,
			...
			message_stream_t &message_data_out
			){
	...
	//create and write hdr + data + footer 
		...
		//ap_axis<128,0,0,0> header_axis;
		//header_axis.data = tmp_header;
		
		message_data_out.write(tmp_header); 

		for (short i = 0; i < size/sizeof(word_t); ++i){
		#pragma HLS pipeline 
#pragma HLS LOOP_TRIPCOUNT min=1 max=256
			message_data_out.write(buff[i]);
			} 
		...
		//ap_axis<128,0,0,0> footer_axis;
		//footer_axis.data = tmp_footer;
		message_data_out.write(tmp_footer); //footer 
...
// (Non-Blocking) send api ==> allows the user to write data on the network without knowing the protocol
size_t send(ap_axis<128,0,0,0> *buff, size_t size, int coord,
			task_id_t task_id, channel_id_t ch_id, 
			hls::stream<ap_axis<128,0,0,0>> &message_data_out
			){
	...
	//create and write hdr + data + footer 
		...
		message_data_out.write(header_axis); 
...
#pragma HLS LOOP_TRIPCOUNT min=1 max=256
		...
		ap_axis<128,0,0,0> footer_axis;
		footer_axis.data = tmp_footer;
		message_data_out.write(footer_axis); //footer 
