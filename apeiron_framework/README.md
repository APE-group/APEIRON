...
This example design demonstrates the main functionalities of the framework, 
using two new hapecom functions: send_gu() and receive_gu(), that allow us to manage
packets with trailing N headers, containing the info (coordinates, channels) of the subsequent communications.


Port 0 of the switch is directly connected to the net_krnl() HLS kernel
that forges this multi_hdrs packet

Port 1-4 of the switch are connected to the acc_krnl() HLS
kernel, through the autogenerated dispatcher_() and
aggregator_().
...
In the latency_test, a packet containing the info of 3 subsequent hops to 3 different switch intranode ports
is sent from the node_0:port_0 to node_1:port_0 where it is received, the hdrs are stored and the communication
is forwarded to the next hops.
...
To launch the test with default parameters:
> ./latency_test -b test_NET4EXA_gu.xclbin -n #number_of_packets
